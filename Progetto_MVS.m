...
Procedure Ostacoli();
  ...
  -- inserire ostacoli  
    for i: indw_t do
        for j: indh_t do
          if((i * j + 2 )% 5 = 0) then
            if ((labirinto[i][j] = Vuoto) & (i != USCITA_X-1 | j != USCITA_Y) & (i != USCITA_X | j != USCITA_Y-1) & (i != USCITA_X+1 | j != USCITA_Y) & (i != USCITA_X | j != USCITA_Y+1)) then   -- se la casella è vuota e l'ostacolo non blocca l'uscita
              labirinto[i][j]:= Ostacolo;       -- inserire ostacolo
            endif;
          endif;
        Endfor;
    Endfor;
